...
function generate_report {
	...
	if [ $dirCount -gt $MAXIMUM_REPORTS ]; then
...
function notify
{
	...
	if [[ $2 -gt $MEM_THRESHOLD  ||  $cpu_usage_int -gt $CPU_THRESHOLD ]]; then
		#Check if process exceeded its CPU or MEM thresholds. If that is the case, send an email to $USER containing the last report
		mail -s "test" conn1278@gmail.com < reports_dir/"$(ls -t reports_dir | head -1)"
	fi
