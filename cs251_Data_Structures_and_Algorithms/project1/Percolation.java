...
public class Percolation {
	public int[][] grid;
	public int size;
	public WeightedQuickUnionUF openSpaces;
	public WeightedQuickUnionUF fullSpaces;
	public Percolation(int n){
		grid = new int[n][n];								//CREATES GRID STRUCTURE, 2D ARRAY
		openSpaces = new WeightedQuickUnionUF(n*n);			//CREATES EMPTY UF, of size n^2 for Open
		fullSpaces = new WeightedQuickUnionUF(n*n);			//Creates EMPTY UF, of size n^2 for Full
		size = n;
	}
	...
	public static void main(String[] args){
		...
		for(int i = n-1; i >= 0; i--){										//UP => DOWN / LEFT => RIGHT SWEEP
			for(int j = 0; j < n; j++){
				if(p.grid[i][j] == 2){
		...
		for(int i = 0; i < n; i++){											//DOWN => UP / RIGHT => LEFT SWEEP
			for(int j = n - 1; j >= 0; j--){
				if(p.grid[i][j] == 2){
					if(i + 1 < n && i + 1 >= 0){							//FILL NORTH IF POSSIBLE
						if(p.grid[i+1][j] == 1){
